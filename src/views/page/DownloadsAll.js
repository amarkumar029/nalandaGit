...
function DownloadsAll () {
  ...
  useEffect(() => {
    // console.log(`${baseUrl}/iqacall?type=${type}`);
    getItemsNotice(`${baseUrl}/all?type=${type}&stype=${stype}`);
  },[type]);
                ...
                <>
                  <li key={key}>
                    <div className="ntitle">
                      <a href={`${user.pdf}`} className="link mt-0" target='_blank' rel="noreferrer">
                        {user.title}
                      </a>
                    </div>
                  </li>
